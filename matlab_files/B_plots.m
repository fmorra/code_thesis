...
% This function generates plots for thr B coefficients. Used to avoid
% wroding the visco_plotter functio too much.
close all; clc;
variables = [alin,a];
...
% Meshing data as in other cases
resolution = 0.25;
...
colorbarlimits = caxisextremes(sd_input,min_lat,max_lat,min_lon,max_lon,depths_to_plot,...
    ...
    coordinate_system);
% Filter data and start saving it in the difference plot matrix
...
for i=1:size(variables,2)
    ...
    % Conversion to Cartesian to 3D interpolate data
    ...
    set(findall(gca, 'type', 'text'), 'visible', 'on')
    grid on;
    % Save plot and difference matrix
